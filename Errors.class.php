 ...
 *
 **/
define('ERR_HANDLER_PATH', substr(dirname(__file__), 0, strpos(dirname(__file__), 'classes') - 1) . "w\\"); // DO NOT change!!
define('ERR_HANDLER_LOG_FOLDER', 'IDB-Plugin-logs'); // Name of logs folder..
define('ERR_HANDLER_ENABLED', 1); // Enable custom error handler?
define('ERR_HANDLER_DISPLAY', 0); // Display a message on screen?
...
define('FILE_ERR_LOG_FILE', 'idb_errors.log'); // File name of error log
define('FILE_FATAL_ERR_LOG_FILE', 'idb_fatal_errors.log'); // File name of fatal error log
...
class Errs {
    ...
    public function log($error, $file) {
        ...
        $file = ERR_HANDLER_PATH . ERR_HANDLER_LOG_FOLDER . '/' . Errs::raStr() . $file;
        $cont = trim($error) . linending() . '***** - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - - - - - - - *****' . linending();

        if (is_dir(ERR_HANDLER_PATH . ERR_HANDLER_LOG_FOLDER)) {
            write($file, $cont);
        }else{
            mkdir(ERR_HANDLER_PATH . ERR_HANDLER_LOG_FOLDER, 0777, true);
            write($file, $cont);
        }
    }
